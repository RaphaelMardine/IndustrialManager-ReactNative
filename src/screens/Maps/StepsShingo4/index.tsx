  ...
  async function getSteps() {
    try {
      const steps = await api.get(
        `/params?paramType=stepFlow&userId=${user._id}`,
      );
      ...
      console.log(err);
      Alert.alert('Aviso', 'Não foi possivel buscar as tarefas');
    }
  }
    ...
    <Main
      ...
      <Container
        ...
        <Content
          ...
          <HeadlineTextLight>Mapa de Shingo 4.0</HeadlineTextLight>
          <View
            style={{
              marginTop: 30,
              backgroundColor: '#fff',
                ...
                <View style={Platform.OS === 'ios' && {marginTop: -30}}>
                  <PickerApp
                    ...
                    onChange={(value: any, index) => {
                      const getLabel = typeStep[index];
                      setStepName(getLabel?.label);
                      setSelectedStepMap(value);
                    }}
                  />
                </View>
              )}
